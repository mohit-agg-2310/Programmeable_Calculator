 ...
 * 
 * @version 1.03	The previous versions plus an improved squareRoot method using Newton Raphon Method, significantFigure 
 *                  and isPositive methods to find out the position of the decimal and the positivity respectively    
 *         
 *@version 1.04     linked with  the calculator project
 *
 *@version 4.0.6	2020-09-10 The JavaFX-based GUI for the implementation of a UNumber calculator with Units
 */
...
UNumber implements Comparable<UNumber> {
	...
	public static int flag=0;      // this variable will be used for testing by tester class
	/**
	 ...
	 * 
	 * @param that is being compare here with the UNumber which is used to call the function
	 * 
	 * @return it returns 1 if that is smaller, -1 if that is greater and 0 if that is equal to this
	 */
	...
	/**********
	 * This sigFig routine tries detect the decimal point in a string and return the index number.  
	 * 
	 * @param str is the string from which the function finds out the decimal point
	 * 
	 * @return i which is the index number of the decimal point's location in the string
	 */
	public int significantFigure(String str)
	{
		int i=0;
		while(i!=str.length())
		{
	...
	/**********
	 * This isPositive routine verifies whether a given string has negative number in it or not
	 * 
	 * @param str is the string which is being verified as positive or negative
	 * 
	 * @return boolean true if the string has a negative sign at the beginning else returns false
	 */
	...
	/**********
	 * This squareRoot routine finds out the square root of a UNumber.  
	 * This function follows Newton Raphons method to find the square root
	 * 
	 * @param originalValue is a UNumber whose square root is being calculated here
	 * 
	 * @return temp is the final result or the square root of the input UNumber
	 */
	public UNumber squareRoot(UNumber originalValue)
	{
		
		UNumber temp = new UNumber(originalValue);		
		UNumber y = new UNumber(2);
		int i=0;
		do
		{
			UNumber x = new UNumber(originalValue);
			UNumber z= new UNumber(temp);
			x.div(z);
			temp.add(x);
			temp.div(y);
			i++;
			System.out.println("The estimation after "+i+" iteration is: " + temp.toDecimalString());
			System.out.println();
